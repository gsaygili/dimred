...
def evaluate_regression (model, X_test, y_te_score, y_te_ind, y_npr, save_folder):
  ...
  ax.set_xticklabels(['NPR (1)', 'NPR (0)',
                    'RF (1)', 'RF(0)'], fontsize=20)
  plt.ylabel("RF/NPR Score", fontsize=20)
  # Creating plot
  bp = ax.boxplot(data)
...
# Calculate the absolute errors for y_npr
  ...
  plt.ylabel("RF Score", fontsize=12)
  ...
  plt.xlabel("Target Score", fontsize=12)
  plt.ylabel("NPR Score", fontsize=12)
  ...
  np.save(save_folder+"y_pred.npy", y_pred)
